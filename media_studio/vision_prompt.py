"""
Creative Vision prompt helpers — shot language, style presets, bridge cues.

Does not invent multi-shot timelines; just compiles a single motion prompt.
"""
...
# Camera / shot helpers (dropdown labels feed the prompt)
SHOT_TYPES: list[str] = [
    "Wide establishing",
    "Slow push-in",
...
# Built-in style presets (editable after insert)
...
def compile_vision_prompt(
    *,
    ...
    """
    Compose a single generation prompt from helpers + free text.

    User free text wins when already rich; helpers prepend camera language.
    """
    parts: list[str] = []
...
def clear_vision_helper_values() -> dict[str, Any]:
    return {
        "shot_type": SHOT_TYPES[1],  # Slow push-in
        "lens": LENS_FEELS[1],
        "motion": MOTIONS[3],  # Push in
